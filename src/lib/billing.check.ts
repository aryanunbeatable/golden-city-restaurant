...
function order(over: Partial<OrderWithItems> & { order_items: OrderItemRow[] }): OrderWithItems {
  return {
...
assert.equal(
  isUnpaid(order({ status: "served", payment_status: "pending", order_items: [] })),
  true,
  "served but unpaid is exactly what billing exists to catch",
);
assert.equal(
  isUnpaid(order({ status: "ready", payment_status: "pending", order_items: [] })),
  true,
  "ready and unpaid is owed too",
);
assert.equal(
  isUnpaid(order({ status: "served", payment_status: "paid", order_items: [] })),
  false,
  "paid is settled, whatever the kitchen thinks",
);
// A voided order is not owed, and an abandoned checkout was never an order.
assert.equal(isUnpaid(order({ status: "cancelled", order_items: [] })), false, "voided is not owed");
assert.equal(
...
const otherTable = order({ source: "table_3", order_items: [item("Dal", 1, 150)] });
...
const voided = order({ source: "table_2", status: "cancelled", order_items: [item("Voided", 1, 500)] });

const all = [round2, round1, otherTable, alreadyPaid, voided];
const bill = tableBill(all, "table_2");

assert.equal(bill.total, 400 + 49 + 20, "both unpaid rounds roll into one total");
assert.equal(bill.itemCount, 2 + 1 + 2, "and every unit is counted, bottles included");
assert.equal(bill.orders.length, 2, "the paid round and the voided one are not owed");
assert.deepEqual(
...
assert.equal(tableBill(all, "table_3").total, 150, "table 3 owes only its own food");

// --- token lookup ------------------------------------------------------------
const parcel = order({ source: "parcel", order_items: [item("Roti", 4, 20)] });
assert.equal(tokenOf(parcel), parcel.id.slice(0, 8).toUpperCase(), "token matches every other screen");

const token = tokenOf(parcel);
assert.equal(findByToken([parcel], token).length, 1, "full token matches");
assert.equal(findByToken([parcel], token.toLowerCase()).length, 1, "case does not matter at the counter");
assert.equal(findByToken([parcel], token.slice(2, 6)).length, 1, "a middle fragment matches — first char is misread most");
assert.equal(findByToken([parcel], " " + token + " ").length, 1, "stray whitespace is forgiven");
assert.equal(findByToken([parcel], "Z").length, 0, "one character is too loose to search on");
assert.equal(findByToken([parcel], "ZZZZ").length, 0, "no match returns nothing");

// A token that was already settled must say "paid", not "not found".
const settled = order({ source: "parcel", payment_status: "paid", payment_method: "counter_cash", order_items: [] });
assert.equal(findByToken([settled], tokenOf(settled)).length, 1, "paid orders are still findable by token");
...
assert.equal(findByToken([abandoned], tokenOf(abandoned)).length, 0, "abandoned checkouts are not findable");

// --- non-table unpaid --------------------------------------------------------
const swiggy = order({ source: "swiggy", payment_status: "pending", order_items: [item("X", 1, 100)] });
const others = otherUnpaid([parcel, swiggy, round1, settled]);
assert.deepEqual(others.map((o) => o.source), ["parcel"], "tables, aggregators and paid orders all excluded");
...
assert.equal(combined.length, 3, "three distinct name+price lines across both rounds");
...
assert.equal(bothPrices.length, 2, "a price change must not merge into one line");
