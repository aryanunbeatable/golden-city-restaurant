...
import { isTableSource, type OrderSource, type OrderWithItems } from "../types/order.ts";

/** Every table, in the order they appear on the counter's tiles. */
export const TABLE_SOURCES: readonly OrderSource[] = ["table_1", "table_2", "table_3", "table_4"];
...
export function billItemCount(orders: OrderWithItems[]): number {
  return orders.reduce((sum, o) => sum + o.order_items.reduce((n, it) => n + it.quantity, 0), 0);
}
 ...
 */
export function tableBill(orders: OrderWithItems[], source: OrderSource): TableBill {
  const mine = orders
    .filter((o) => o.source === source && isUnpaid(o))
    .sort((a, b) => a.created_at.localeCompare(b.created_at));
  return { source, orders: mine, total: billTotal(mine), itemCount: billItemCount(mine) };
}
...
export function combinedLines(orders: OrderWithItems[]): BillLine[] {
  ...
  for (const o of orders) {
    for (const it of o.order_items) {
      ...
      else byKey.set(key, { name: it.item_name, qty: it.quantity, unitPrice: it.unit_price });
    }
  }
  return [...byKey.values()];
}
 ...
 */
export function findByToken(orders: OrderWithItems[], query: string): OrderWithItems[] {
  const needle = query.trim().toUpperCase();
  if (needle.length < 2) return [];
  return orders
    .filter((o) => o.status !== "awaiting_payment" && tokenOf(o).includes(needle))
...
export function otherUnpaid(orders: OrderWithItems[]): OrderWithItems[] {
  return orders
    .filter((o) => isUnpaid(o) && !isTableSource(o.source) && o.source !== "swiggy" && o.source !== "zomato")
    .sort((a, b) => a.created_at.localeCompare(b.created_at));
}
